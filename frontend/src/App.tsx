...
const App: React.FC = () => {
  ...
  const [startedRecipe, setStartedRecipe] = useState<any | null>(null);
  ...
  const handleStartRecipe = async (suggestion: Suggestion) => {
    setStepLoading(true);
    setStepError(null);
    try {
      const recipe = await fetchRecipeDetail(suggestion.recipe.id);
      setStartedRecipe(recipe);
      setChatStep(0);
      setChatbotOpen(true);
    } catch (err: any) {
      setStepError('Failed to load recipe steps.');
    } finally {
      setStepLoading(false);
    }
  };

  // Removed unused handleCloseModal and setViewingSuggestion
  ...
  const handleGetSuggestions = useCallback(async () => {
    ...
    try {
      ...
      setError(err.message || 'Failed to fetch recipe suggestions. Please try again later.');
      console.error(err);
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-800 font-sans">
      ...
      {/* Step-by-step process display at bottom */}
      {/* Mini Chatbot Modal for Step-by-Step Instructions */}
      {chatbotOpen && startedRecipe && (
        <div className="fixed inset-0 bg-black bg-opacity-40 z-50 flex justify-center items-center">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6 flex flex-col">
            <h2 className="text-2xl font-bold mb-4 text-green-700">Recipe Chatbot</h2>
            <h3 className="text-lg font-semibold mb-2">{startedRecipe.title}</h3>
            <div className="flex-grow mb-4">
              {startedRecipe.instructions.split(/\n|\r|\./).filter(Boolean)[chatStep] ? (
                <div className="text-lg text-gray-800">
                  <span className="font-bold">Step {chatStep + 1}:</span> {startedRecipe.instructions.split(/\n|\r|\./).filter(Boolean)[chatStep].trim()}
                </div>
              ) : (
                <div className="text-lg text-green-700 font-bold">All steps completed! Enjoy your meal!</div>
              )}
            </div>
            <div className="flex gap-2 justify-end">
              ...
              {chatStep > 0 && (
                <button
                  className="px-4 py-2 rounded bg-green-100 hover:bg-green-300 text-green-700 font-semibold"
                  onClick={() => setChatStep(chatStep - 1)}
                >Back</button>
              )}
              {chatStep < startedRecipe.instructions.split(/\n|\r|\./).filter(Boolean).length - 1 && (
                <button
                  className="px-4 py-2 rounded bg-green-500 hover:bg-green-700 text-white font-semibold"
                  onClick={() => setChatStep(chatStep + 1)}
                >Next</button>
              )}
            </div>
